...
const GeneSequence = (
    ...
    const [sliderValue, setSliderValue] = useState({ start: 0, end: 100 })
    const [isDraggingStart, setIsDraggingStart] = useState(false)
    const [isDraggingEnd, setIsDraggingEnd] = useState(false)
    const [isDraggingRange, setIsDraggingRange] = useState(false)
    const [hoverPosition, setHoverPostion] = useState<number | null>(null)
    const [mousePosition, setMousePostion] = useState<{ x: number, y: number } | null>(null)
    ...
    useEffect(() => {
        ...
        if (isNaN(startNum) || isNaN(endNum) || totalSize <= 0) {
            setSliderValue({ start: 0, end: 100 })
            return
        }
    ...
    useEffect(() => {
        const handleMouseMove = (e: MouseEvent) => {
            ...
            const currentEndtNum = parseInt(endPosition)

            if (isDraggingStart) {
                if (!isNaN(currentEndtNum)) {
                    if (currentEndtNum - newPosition + 1 > maxViewRage) {
                        onStartPositionChange(String(currentEndtNum - maxViewRage + 1))
                    } else if (newPosition < currentEndtNum) {
                        onStartPositionChange(String(newPosition))
                    }
                }
            } else if (isDraggingEnd) {
                if (!isNaN(currentStartNum)) {
                    if (newPosition - currentStartNum + 1 > maxViewRage) {
                        onEndPositionChange(String(currentStartNum - maxViewRage + 1))
                ...
                const dragPixels = relativeX - dragStartX.current.x
                const dragDeltaBases = Math.round(dragPixels / pixelsPerBase)
    ...
    const handleRangeMouseDown = useCallback((e: React.MouseEvent) => {
        ...
        const startNum = parseInt(startPosition)
        const endtNum = parseInt(endPosition)

        if (isNaN(startNum) || isNaN(endtNum)) return
        ...
        for (let i = 0; i < sequenceData.length; i += BASES_PER_LINE) {
            ...
            lines.push(
                <div
                    ...
                    className="flex"
                >
                    <div className="mr-20 w-20 text-rigth text-gray-500 select-none">
                        {lineStartProps.toLocaleString()}
                    </div>
        ...
        <Card className="gap-0 border-none bg-white py-0 shadow-sm">
            ...
            <CardContent className="pb-4">
                {geneBounds && (
                    <div className="mb-4 flex flex-col">
                        <div className="mb-2 flex flex-col items-center justify-between text-xs sm:flex-row">
                            <span className="text-[#3c4f3d]/70 flex items-center gap-1">
                            ...
                            <div className="relative">
                                ...
                                >
                                    <div className="absolute top-1/2 h-2 w-full -translate-y-1/2 rounded-full bg-[#e9eeea]">
                                        <div
                                            className="absolute top-1/2 h-2 w-full -transalate-y-1/2 cursor-grab rounded-full bg-[#3c4f3d] activate:cursor-grabbing"
                                            style={{
                                        ...
                                        <div
                                            className="absolute top-1/2 flex h-6 w-6 -translate-x-1/2 -translate-y-1/2 cursor-grab items-center justify-center rounded-full border-[#3c4f3d] bg-white shadow active:cursor-grabbing"
                                ...
                                <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-9">
                                    <div className="flex items-center gap-2">
                                        <span className="tex-xs text-[#3c4f3d]/70">Start:</span>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className="tex-xs text-[#3c4f3d]/70">End:</span>
                ...
                <div className="mb-2 flex items-center justify-between text-xs">
                    <span className="text-[#3c4f3d]/70">
                        {
                            geneDetail?.genomicinfo?.[0]?.strand === '+' ? "Forward strand (5' -> 3')"
                                : geneDetail?.genomicinfo?.[0]?.strand === '-' ? "Reverse strand (3' <- 5')" : 'Strand Information not avaible'
                        }
                    </span>

                    <span className="text-[#3c4f3d]/70">
                        Maximum Window Size: {maxViewRage.toLocaleString()} bp
                    </span>
                </div>
                ...
                <div className="w-full rounded-md bg-[#e9eeea]/50 p-3">
                    {isLoading ? (
                        ...
                        <p className="text-center text-sm text-[#3c4f3d]/60">
                            {error ? 'Error laoding Sequence' : 'No Sequence Data loaded'}
                        </p>
                    )}
                </div>
                ...
                {hoverPosition !== null && mousePosition !== null && (
                    <div
                        className="pointer-events-none fixed z-50 bg-[#3c4f3d] px-2 py-1 text-white shadow-md"
                        style={{ 
