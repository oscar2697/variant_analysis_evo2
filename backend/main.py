...
app = modal.App('gen-analysis', image=evo2_image)
...
def run_brca1_analysis():
    ...
    return {"variants": brca1_subset.to_dict(orient='records'), "plot": plot_data, "auroc": float(auroc)}
...
def brca1_example():
    ...
    if "plot" in result:
        ...
        img = mpimage.imread(BytesIO(plot_data))
...
def get_genome_sequence(position, genome: str, chromosome: str, window_size=8192):
    ...
    print(
        f"Fetching {window_size} bp window around position  {position} from USC API...")
    ...
    if response.status_code != 200:
        raise Exception(f"Error fetching sequence: {response.status_code}")
    ...
    if len(sequence) != expected_length:
        print(
            f"Warning: Received sequence length({len(sequence)}) for expected length({expected_length})")

    print(
        f"Loaded reference genome squence window  (length: {len(sequence)} bases)")
...
def analyze_variant(relative_pos_in_window, reference, alternative, window_seq, model):
    ...
    ref_score = float(model.score_sequences([window_seq])[0])
    var_score = float(model.score_sequences([var_seq])[0])

    delta_score = var_score - ref_score

    threshold = 0.00083237886
    lof_std = 0.0014333354
    func_std = 0.000889649
...
class Evo2Model:
    ...
    def analyze_single_variant(self, variant_position: int, alternative: str, genome: str, chromosome: str):
        ...
        print(
            f"Fetched genomic window sequence, first 100 bases: {window_seq[:100]}")
        ...
        reference = window_seq[relative_position]
